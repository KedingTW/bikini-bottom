...
### 短期（優先）
- [ ] **升級 OpenAB 0.8.4** — Trusted bot @mention bypass involvement gate，改善 multi-agent 協作（bot 互 mention 不再被擋）
  - 改 Dockerfile `FROM ghcr.io/openabdev/openab:0.8.4`
  - 設定各 bot 的 `trusted_bot_ids`
  - 測試 bot-to-bot mention 無需人類介入即可觸發

### 長期 / 未來規劃
- [ ] **OpenAB Lifecycle Hooks (pre_boot)** — 用 `[hooks.pre_boot]` 取代 entrypoint-wrapper.sh，等 k3s 計畫完成後再做
- [ ] Skill 遷移評估（見下方計畫）
...
<!-- 🐋 珍珍 GitHub 顯示測試 - 2026-06-02 06:15 -->
