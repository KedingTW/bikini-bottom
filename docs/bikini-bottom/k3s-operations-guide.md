...
| **資源隔離（Resource Requests）** | 每個 Pod 設定最低資源保證，搭配監控主動管理 |
...
| **Resource Requests** | CPU/RAM 最低保證 | 告訴 K3s 排程時需要留多少資源給這個 Pod |
...
# 如果是常態性過高，降低 session pool
# 調整 config.toml 中的 max_sessions / session_ttl_hours
```
...
> ⚠️ **規範：所有 Deployment 一律不設 `resources.limits`，只設 `resources.requests`。**

原因：
- Agent 處理大任務（大量 MCP tools、長 session）時記憶體需求波動很大
- 設了 limits 會導致 kiro-cli 被 OOM Kill，產生 Broken pipe (OS Error 32)
- 單節點環境下 limits 沒有隔離意義，反而增加故障率
...
不靠 limits 硬殺，而是：
1. `kubectl top pods -n bikini-bottom --sort-by=memory` 找出誰吃最多
2. `kubectl rollout restart deployment/<name> -n bikini-bottom` 重啟釋放
3. 如果是常態性過高，檢查 session pool 設定（`max_sessions` / `session_ttl_hours`）
