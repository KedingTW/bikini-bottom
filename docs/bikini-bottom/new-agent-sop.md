...
在 NAS 上建立該角色的 projects 目錄（entrypoint 會自動 symlink）：

```bash
# 進任意已運行的 agent 容器操作 NAS
kubectl exec deployment/bob -n bikini-bottom -- mkdir -p /nas/agents/krabs/projects
kubectl exec deployment/bob -n bikini-bottom -- chown -R agent:agent /nas/agents/krabs/
```

如果沒做這步，容器會 fallback 用本地 hostPath 的 projects 目錄，可能有權限問題導致 kiro-cli 無法啟動。

## 認證

kiro-cli 透過 `KIRO_API_KEY` 環境變數自動認證，**不需要手動 login**。

確認 K8s secret `kiro-api-keys` 中有該角色的 key 即可：

```bash
# 確認 secret 中有該角色的 key
kubectl get secret kiro-api-keys -n bikini-bottom -o jsonpath='{.data}' | python3 -c "import sys,json; print('\n'.join(sorted(json.load(sys.stdin).keys())))"

# 如果沒有，新增（所有角色共用同一組 key）：
kubectl get secret kiro-api-keys -n bikini-bottom -o json | \
  python3 -c "import sys,json,base64; s=json.load(sys.stdin); s['data']['KRABS']=base64.b64encode(b'你的key').decode(); json.dump(s,sys.stdout)" | \
  kubectl apply -f -
```

同時確認 `config.toml` 的 `[agent]` 區塊有設定：
```toml
