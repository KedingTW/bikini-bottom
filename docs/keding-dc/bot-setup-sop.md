...
- 糾正紀錄機制（寫入 `/home/agent/correction-log.md`）
- 輸出風格：只輸出答案、不輸出解析
...
關鍵差異點：
- `command: ["/usr/local/bin/entrypoint-minimal.sh"]` — 用精簡 entrypoint
...
| 項目 | 比奇堡 | 科定DC |
|------|--------|--------|
| Entrypoint | `entrypoint-wrapper.sh` | `entrypoint-minimal.sh` |
| NAS mount | 需要（projects symlink） | 不需要 |
