<template>
  <!-- Mobile: full width list, hidden when agent selected -->
  <div :class="[
    selected && !forceShow ? 'hidden md:block' : '',
    ...
    <div v-else class="py-1">
      <button v-for="a in agents" :key="a.name" @click="$emit('select', a)"
        class="w-full flex items-center gap-2.5 px-4 py-3 md:px-3 md:py-2.5 text-left transition"
        :class="selected?.name === a.name ? 'bg-cyan-500/15 border-l-2 border-cyan-400' : 'hover:bg-white/5 border-l-2 border-transparent'">
        <img v-if="a.avatar_url" :src="a.avatar_url" class="w-9 h-9 md:w-8 md:h-8 rounded-full object-cover border border-white/20">
        <img v-else :src="'/avatar/' + a.name" class="w-9 h-9 md:w-8 md:h-8 rounded-full object-cover border border-white/20" @error="$event.target.style.display='none'">
        <div class="flex-1 min-w-0">
          <div class="text-sm font-medium truncate" :class="selected?.name === a.name ? 'text-cyan-300' : 'text-white/90'">{{ a.display }}</div>
          <div class="text-[11px] md:text-[10px] text-white/50 truncate">{{ a.role }}</div>
        </div>
        <span class="text-white/30 md:hidden">›</span>
      </button>
    </div>
  </div>
</template>
